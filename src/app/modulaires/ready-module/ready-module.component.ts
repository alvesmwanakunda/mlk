...
@Component({
  selector: 'app-ready-module',
  templateUrl: './ready-module.component.html',
  styleUrls: ['./ready-module.component.scss']
})
export class ReadyModuleComponent implements OnInit,AfterViewInit {
  ...
  if(this.user?.user?.role=='user'){
      this.projectService.getAllModuleByEntreprisePp(this.user?.user?.entreprise).subscribe((data:any)=>{
      ...
      this.projectService.getAllModulePp().subscribe((data:any)=>{
        this.dataSource.data =data.message.map((data)=>({
          id:data?._id,
          nom:data?.nom,
          projet:data?.project?.projet,
