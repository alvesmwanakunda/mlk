...
export class StockModuleComponent implements OnInit,AfterViewInit {
  ...
  if(this.user?.user?.role=='user'){
      this.projectService.getAllModuleByEntrepriseStock(this.user?.user?.entreprise).subscribe((data:any)=>{
      ...
      this.projectService.getAllModuleStock().subscribe((data:any)=>{
        this.dataSource.data =data.message.map((data)=>({
          id:data?._id,
          nom:data?.nom,
          projet:data?.project?.projet,
