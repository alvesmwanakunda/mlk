...
@Component({
  selector: 'app-move-plan',
  templateUrl: './move-plan.component.html',
  styleUrls: ['./move-plan.component.scss']
})
export class MovePlanComponent implements OnInit {
    ...
    constructor(
      ...
      public dialogRef:MatDialogRef<PlanModuleComponent>,
    ...
    getAllFiles(){
      this.projetService.getAllDocuments(this.data.idModule).subscribe((res:any)=>{
        if(this.data.extension){
    ...
    getFile(){
      this.projetService.getPlanModule(this.data?.id).subscribe((res:any)=>{
    ...
    getFolder(){
      this.projetService.getFolderId(this.data?.id).subscribe((res:any)=>{
        this.infos = res.message;
      },(error)=>{
        console.log('Error', error);
      })
    ...
    getAllSubFolder(idFolder){
      this.projetService.getFolderDetailId(idFolder).subscribe((res:any)=>{
   ...
   moveFolder(id,parent){
    this.projetService.moveFolder(id,parent).subscribe((res:any)=>{
   ...
   moveFile(id,parent){
    this.projetService.moveFile(id,parent).subscribe((res:any)=>{
       console.log("Response", res);
