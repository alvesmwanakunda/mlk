import { NgModule } from '@angular/core';
import { CommonModule } from '@angular/common';
import { StatiTachesRoutingModule } from './stati-taches-routing.module';
import { SharedModule } from '../shared/shared.module';
import { NavbarModule } from '../navbar/navbar.module';
import { AuthGuardService } from '../shared/services/auth-guard.service';
import { JwtModule, JWT_OPTIONS } from '@auth0/angular-jwt';
import { StatiTachesComponent } from './stati-taches.component';
import { UpdateTachesModule } from '../taches/update-taches/update-taches.module';

export function jwtOptionsFactory() {
  return {
    tokenGetter: () => {
      const token = JSON.parse(localStorage.getItem('user'))?.token;
      console.log("Token", token);
      return token || '';
    },
    allowedDomains: [window.location.origin], // Dynamiquement obtenu
    disallowedRoutes: [`${window.location.origin}/statistique/tache`], // Dynamiquement obtenu
  };
}


@NgModule({
  declarations: [StatiTachesComponent],
  imports: [
    CommonModule,
    StatiTachesRoutingModule,
    SharedModule,
    NavbarModule,
    UpdateTachesModule,
    JwtModule.forRoot({
      jwtOptionsProvider: {
        provide: JWT_OPTIONS,
        useFactory: jwtOptionsFactory,
      },
    }),
],
 providers:[AuthGuardService]
})
export class StatiTachesModule { }
